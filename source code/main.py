...
def model(z, t):
    ...
    R = -2*pow(np.e, 3*t) - 2* pow(np.e, 6*t)
    J = -2*pow(np.e, 3*t) -  pow(np.e, 6*t)
...
plt.title("LOVE BETWEEN AN EAGER BEAVER AND AN EAGER BEAVER", fontsize=10)
...
ax.set_title("LOVE BETWEEN AN EAGER BEAVER AND AN EAGER BEAVER", fontsize = 10)
